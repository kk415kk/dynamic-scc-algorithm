...
# foo = LB.Node('foo')
# qux = LB.Node('qux')
# baz_foo = LB.Edge(baz, foo)
# foo_qux = LB.Edge(foo, qux)
# foo_baz = LB.Edge(foo, baz)
# G.optimized_add_edges(set([baz_foo, foo_qux, foo_baz]))
...
# e19 = LB.Edge(q,o)
# e20 = LB.Edge(q,l)
# G.optimized_add_edges(set([e12, e13, e14, e15, e16, e17, e18, e19, e20]))
...
for node in G.inverse_components:
  print "%s: %s" % (node, G.inverse_components[node])
print ""
